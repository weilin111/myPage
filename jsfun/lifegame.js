...
function add_game_canvas_to_container(container_id) {
    ...
    class Game {
        ...
        constructor(ctx, canvas) {
            ...
            this.simulator = new lifegameLab(this)
    ...
    class lifegameLab {
        ...
        constructor(game) {
            ...
            this.graph_list.push(new lifegame01() )
    ...
    class lifegame01{
        ...
        constructor(draw_info){
            ...
            console.log(this.state_grid)

            let n_alive_seeding=this.n_col*this.n_row/200 *getRandomInt(1,6)
            // for (let i = 0; i < n_alive_seeding; i++) {
            //     let grid=this.get_random_grid()
            //     grid.state="alive"
            // }


            let center_rigion=[]
            let bia=25+getRandomInt(2,10)

            let check=(grid)=>{
                let res=false
                if (grid.row>bia && grid.row<this.n_row-bia){
                    if( grid.col>bia && grid.col<this.n_col-bia){
                        res=true
                    }
                }
                return res
            }
            center_rigion=this.state_grid.filter(  e=>check(e)    )

            n_alive_seeding=50 *getRandomInt(1,6)
            for (let i = 0; i < n_alive_seeding; i++) {
                let grid=center_rigion[   getRandomInt(0,center_rigion.length-1) ]
                grid.state="alive"
            }
        ...
        get_random_grid(){
            ...
            let i=getRandomInt(0,this.n_row*this.n_col-1)

            return this.state_grid[i]
        ...
        update_grid_by_rule(){
            ...
            this.state_grid.forEach(
                g=>{
                    if(g.state=="alive"){  this.rule_for_alive(g)  }
                    else if(g.state=="dead"){  this.rule_for_dead(g)  }
                }
            )

            // this.fix_alive_grid.forEach(
            //     g=>{
            //         g.state="alive"
            //     }
            // )
        ...
        rule_for_dead(grid_point){

            let surrounding_grid=this.get_surrounding_grid(grid_point)

            let n_surrounding_alive=0
            surrounding_grid.forEach(
                g=>{
                    if(g.state=="alive"){  n_surrounding_alive+=1 }
                }
            )

            // if (n_surrounding_alive==2){  if (Math.random()>0.85){grid_point.state="alive"}   }
            if (n_surrounding_alive==3){  if (Math.random()>0.15){grid_point.state="alive"}   }
            
        }

        rule_for_alive(grid_point){
            ...
            if (n_surrounding_alive<2){  grid_point.state="dead"   }
            if (n_surrounding_alive>3){  grid_point.state="dead"   }
        ...
        rule_for_random(){
            this.get_random_grid().state="alive"
        }


        rule_for_fix_alive(grid_point){

        }


        get_surrounding_grid(grid){
            let res=[]

            let {index,row,col} = grid

            let n_col=this.n_col
            let index_offset=[  -1,1,
                                -1+n_col,n_col,n_col+1,
                                -1-n_col,0-n_col,1-n_col,
                               
              ]
            
            index_offset.forEach(
                e=>{
                    if(index+e<this.n_row*this.n_col  &&  index+e>=0 ){
                        res.push(  this.state_grid[index+e]    )
                    }
                }
            )
        ...
        draw_grid(){
            ...
            let y0=this.draw_info.y
            let dx=this.draw_info.width/this.n_col
            let dy=this.draw_info.width/this.n_row

            for (let i = 0; i < this.n_row*this.n_col; i++) {


                        let grid_point=this.state_grid[i]
                        let row=grid_point.row
                        let col=grid_point.col

                        if (grid_point.state=="dead"){   ctx.fillStyle=this.color }
                        if (grid_point.state=="alive"){   ctx.fillStyle=this.color2 }

                        ctx.fillRect(  x0+col*dx,y0+row*dy,dx,dy  )    
