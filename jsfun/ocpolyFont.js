...
function add_game_canvas_to_container(container_id) {
    ...
    class Game {
        ...
        constructor(ctx, canvas) {
            ...
            this.simulator = new ocpolyFontLab(this)
    ...
    class ocpolyFontLab {
        ...
        constructor(game) {
            ...
            this.cannon_world.gravity = new CANNON.Vec3(0, g, 0)


            this.update_random_platform_datas()
            ...
            bullet_gun.gun_info.x=canvas.width *Math.random()
            bullet_gun.gun_info.y=canvas.height *Math.random()
            bullet_gun.font_base0=bullet_gun.font_base2
        ...
        update_random_platform_datas(){

            this.platform_datas=[]
            let n=getRandomInt(1,15)
            for (let i = 0; i < n; i++) {
                
                let angle=getRandomInt(0,360)/360*3.1415*2
                let x= canvas.width*Math.random()
                let y=canvas.height*Math.random()

                let width=canvas.width/3  *Math.random()
                let height=canvas.height/10 *Math.random()
                
                this.platform_datas.push(
                    {
                        mass: 0,
                        position: new CANNON.Vec3(x, y, 0),
                        velocity: new CANNON.Vec3(width, height, 0),
                        shape: new CANNON.Box(new CANNON.Vec3(200, 10, 1000)),
                        quaternion: new CANNON.Quaternion(0,0,Math.cos(angle),Math.sin(angle)  )
                    }
    ...
    class BulletGun{
        ...
        update(){
            ...
            if(this.update_timer>this.max_update_timer){
                ...
                let dx = this.gun_info.dx - 2*(Math.random()-0.5)
                let dy =this.gun_info.dy -  2*(Math.random()-0.5)
                let l = Math.sqrt(dx * dx + dy * dy)
                let speed = 1
        ...
        draw(){
            ...
            ctx.fillText(`${this.gun_info.x.toFixed(1)} ${this.gun_info.y.toFixed(1)}`,this.gun_info.x,  this.gun_info.y)
        }
