...
window.onload = () => {
    ...
    archive_dom_list = $(".achives-post-list-item-container")
    ...
    var resolve_data = (archive_dom) => {
        let res = {
            date: "",
            title: "",
            x: "",
            y: "",   // number until the date
        }
    ...
    console.log(archive_dom_list[0])
    for (let index =  archive_dom_list.length-1; index >=0; index--) {
        data_list.push(resolve_data( archive_dom_list[index] ))
    }
        ...
        draw() {
            ...
            this.point_list.forEach(
                (point, i) => {
                    ...
                    let angle=Math.PI*15/180
                    ctx.save()
                    ctx.translate(draw_x, draw_y)
                    ctx.rotate(angle)
                    ctx.translate(-1 * draw_x, -1 *draw_y)
                    
                    ctx.font=font0
                    ctx.fillText(point.date+"  "+point.title,draw_x,draw_y)  //!!  2022年11月19日
