...
function add_game_canvas_to_container(container_id) {
    ...
    class Game {
        ...
        constructor(ctx, canvas) {
            ...
            this.simulator = new mapGeneratorLab(this)
    ...
    class mapGeneratorLab {
        ...
        constructor(game) {
            ...
            this.graph_list.push(new mapGenerator() )
    ...
    class mapGenerator{
        ...
        draw_info = {
            ...
            y: canvas.height * 0.1,
            width: this.width,
            height: this.width * canvas.height/canvas.width ,
        }
        ...
        update_timer=0
        update_timer_max=getRandomInt(2,30)
        ...
        constructor(draw_info){
            ...
            this.out_box={
            left:this.draw_info.x,
            right:this.draw_info.x+this.draw_info.width,
            up:this.draw_info.y+this.draw_info.height,
            down:this.draw_info.y,}

            this.width = this.draw_info.width

            this.room_generation_info={
                width_range:[this.width*0.02,this.width*0.2],  //include x y direction
                max_conenction_number:5
            }
    
            this.bridge_generation_info={
                width_range:[this.width*0.02*0.1,this.width*0.3*0.1]  //include x y direction
            }




            let width=random_value(   this.room_generation_info.width_range  )
            let height=random_value(   this.room_generation_info.width_range  )
            let init_room={
                left: this.draw_info.x +this.draw_info.width/2 ,
                right:this.draw_info.x +this.draw_info.width/2 + width,
                down: this.draw_info.y+this.draw_info.height/2,
                up:   this.draw_info.y+this.draw_info.height/2 + height,
                color:get_random_Color(),
                lineWidth:getRandomInt(4,9),
                bridge_list:[]
            }
            this.room_list.push(init_room)



            console.log(this.bridge_list,this.room_list)
        ...
        generation(){
            ...
            let {bridge,room}=this.propose()

            // if (this.check_out(bridge)==false  && this.check_overlap(bridge)==false ){
            //     this.bridge_list.push(bridge)
            // }
            // if (this.check_out(room)==false  && this.check_overlap(room)==false ){
        ...
        propose(){
            ...
            let base_room=random_select(this.room_list)

            let direction=random_select(this.direction_list)

            let direction_op_dict={
                "left" : "right" ,
                "right": "left",
                "up"   : "down" ,
                "down" : "up" 
            }
            let direction2=direction_op_dict[direction]

            let bridge=this.room2room(base_room,direction,this.bridge_generation_info.width_range)
            let room  =this.room2room(bridge,   direction,this.room_generation_info.width_range)


            return {bridge:bridge,room:room}

        }


        room2room(room,direction,range){
            let res={
                left: 0,
                right:0,
                down: 0,
            ...
            if (direction=="left") {
                res.right=base.left
                res.left =base.left - random_value(range)
                let range2=random_range( [base.down,base.up], random_value(range)   )
                res.up= range2[1]
                res.down= range2[0]
            }
            ...
            if (direction=="right") {
                res.left =base.right
                res.right=base.right + random_value(range)
                let range2=random_range( [base.down,base.up], random_value(range)   )
                res.up= range2[1]
                res.down= range2[0]
            }
            ...
            if (direction=="up") {
                res.down =base.up
                res.up   =base.up + random_value(range)
                let range2=random_range( [base.left,base.right], random_value(range)   )
                res.right= range2[1]
                res.left= range2[0]
            }
            ...
            if (direction=="down") {
                res.up =base.down
                res.down   =base.down - random_value(range)
                let range2=random_range( [base.left,base.right], random_value(range)   )
                res.right= range2[1]
                res.left= range2[0]
            }
            
            // console.log( random_value(range))
        ...
        check_out(rect){
            ...
            let res=this.is_out(rect,this.out_box)
        ...
        check_overlap(rect){
            ...
            let res=false
             ...
             this.room_list.forEach(
                e=>{
                    if (this.is_overlap(rect,e)){res=true}
                }
             )
             this.bridge_list.forEach(
                e=>{
        ...
        constructor(draw_info){
            ...
            for (let i = 0; i < n_row; i++) {
                ...
                for (let j = 0; j < n_col; j++) {
                    ...
                    let temp_grid=new mapGenerator(draw_info)
